...
app.listen(13333, async () => {
    ...
    const r1 = await fetchTest("http://localhost:13333/.test/index.html");
    console.log("dotdir status: " + r1.status);

    // Try to access a dotfile (.test.txt)
    const r2 = await fetchTest("http://localhost:13333/.test.txt");
    console.log("dotfile status: " + r2.status);

    // Normal file should still work
    const r3 = await fetchTest("http://localhost:13333/index.html");
