...
//
// send looks for "<path>.<ext>" after a stat that failed with ENOENT and after nothing else: a path
// running through a file (ENOTDIR) or a name too long for the filesystem is reported as it was
// asked for. Trying the extensions anyway reported the error of the last one tried, a path the
// request never named. Found by `npm run fuzz`, seed 3561875562.
...
app.use((err, req, res, next) => {
    // the path inside the message is this machine's, and only what follows the root says which
    // file was looked for
