// must write every header the application set exactly once, in the order it set them, however many
// there are and whatever they hold. ASCII values only: a non-ascii one leaves node as latin1 and
// uWS as utf-8, which CONTRIBUTING keeps out of every comparison
...
// the ordinary path, where the head goes out line by line: a compiled route writes its own
...
// The lines each server writes its own way, left out: node puts Connection, Keep-Alive and Date
// after the application's, uWS writes the framing last and a mark of its own, and x-powered-by is
// on by default in Express only. Names are lowercased, node keeps the case it was given
...
// res.set stores false for a type nobody knows, and end() writes it as it stands
