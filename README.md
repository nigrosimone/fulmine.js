...
- `file cache`, on by default. Small files served by `res.sendFile` come from a bounded in-process cache, checked against the file's `stat` on every request, so an edited file is never served stale. The check is free: it reads the `stat` the request already paid for rather than taking one of its own, and a file whose mtime or size moved is read again. Turn it off where every request has to reach the disk whatever the `stat` says: it was worth about 4% on a 4KB file here, so the cost of turning it off is small.
